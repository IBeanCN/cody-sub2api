export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexUserAgentHint: 'The full Codex User-Agent used for all outbound requests, for customizing the OS / arch / terminal fingerprint. Leave empty to build the standard codex-tui identity from the version below (recommended). If set, both the leading and trailing version declarations are synchronized to the version below, so the UA never stays pinned to the release entered here.',
        openaiCodexUserAgentDesktopHint: 'Desktop mode does not use this global UA: the outbound identity is always the two-version Desktop form (leading segment + official trailer), with the OS / arch fingerprint segment matching the real client sample, avoiding a mixed identity from CLI fingerprints.',
