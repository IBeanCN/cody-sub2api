export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexUserAgentHint: '出站统一使用的完整 Codex User-Agent，用于自定义 OS / 架构 / 终端指纹。留空则按下方版本号拼出标准 codex-tui 形态（推荐）。填写后首段和尾部的版本号仍会被下方版本号同步覆盖，避免这条 UA 停在填写时的旧版本。',
        openaiCodexUserAgentDesktopHint: 'Desktop 模式不使用此全局 UA：出站身份固定为「Desktop 首段 + 官方尾组」双版本形态，OS / 架构指纹段保持与真实客户端抓包一致，避免 CLI 指纹混入产生混合身份。',
